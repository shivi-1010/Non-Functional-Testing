...
declare -A packages=( ["httpd"]="2.4" ["php"]="7.2" ["python3.7"]="3.7" ["postgresql"]="14" )

# Check if each package is installed and if the correct version is set to default
for package in "${!packages[@]}"; do
    version=$(rpm -q --queryformat '%{VERSION}' $package)
    if [ $? -eq 0 ]; then
        echo "$package is installed."
        if [[ $version == ${packages[$package]}* ]]; then
            echo "The correct version of $package is set to be default."
        else
            echo "The incorrect version of $package is set to be default. Expected version: ${packages[$package]}, Installed version: $version"
        fi
    else
...
python_libraries=("beautifulsoup4" "freezegun" "ll-xist" "lxml" "pillow" "psycopg2" "python-ldap" "Sphinx")
for library in "${python_libraries[@]}"; do
    python3.7 -c "import $library" 2> /dev/null
    if [ $? -eq 0 ]; then
        echo "Python library $library is installed."
    else
        echo "Python library $library is not installed."
    fi
done
